...
{
    //qCDebug(KHTML_LOG) << "parser constructor";
#if SPEED_DEBUG > 0
    qt.start();
#endif
...
{
#if SPEED_DEBUG > 0
    qCDebug(KHTML_LOG) << "TIME: parsing time was = " << qt.elapsed();
#endif
...
void KHTMLParser::parseToken(Token *t)
{
    if (t->tid > 2 * ID_CLOSE_TAG) {
        // qCDebug(KHTML_LOG) << "Unknown tag!! tagID = " << t->tid;
        return;
    }
    if (discard_until) {
    ...
    qCDebug(KHTML_LOG) << "inline=" << m_inline << " inBody=" << inBody << " haveFrameSet=" << haveFrameSet << " haveContent=" << haveContent;
#endif
    ...
    if (t->tid == ID_TEXT && t->text) {
        ...
        qCDebug(KHTML_LOG) << "length=" << t->text->l << " text='" << QString::fromRawData(t->text->s, t->text->l) << "'";
#endif
    }
...
#ifdef PARSER_DEBUG
        qCDebug(KHTML_LOG) << "t->id: " << t->tid << " is forbidden :-( ";
#endif
        popOneBlock();
    }
...
#ifdef PARSER_DEBUG
        qCDebug(KHTML_LOG) << "insertNode failed current=" << current->id() << ", new=" << n->id() << "!";
#endif
        if (map == n) {
#ifdef PARSER_DEBUG
            qCDebug(KHTML_LOG) << "  --> resetting map!";
#endif
            map = nullptr;
        }
        if (form == n) {
#ifdef PARSER_DEBUG
            qCDebug(KHTML_LOG) << "   --> resetting form!";
#endif
            form = nullptr;
        }
        delete n;
...
bool KHTMLParser::insertNode(NodeImpl *n, bool flat)
{
    ...
    if (newNode) {
#ifdef PARSER_DEBUG
        qCDebug(KHTML_LOG) << "added " << n->nodeName().string() << " to " << tmp->nodeName().string() << ", new current=" << newNode->nodeName().string();
#endif
...
#ifdef PARSER_DEBUG
        qCDebug(KHTML_LOG) << "ADDING NODE FAILED!!!! current = " << current->nodeName().string() << ", new = " << n->nodeName().string();
#endif
        // error handling...
        HTMLElementImpl *e;
        bool handled = false;
...
#ifdef PARSER_DEBUG
                    qCDebug(KHTML_LOG) << "adding style before to body failed!!!!";
#endif
        ...
        case ID_TR:
            ...
            default: {
                ...
                if (n->isTextNode() ||
                    ...
                    int exceptioncode = 0;
#ifdef PARSER_DEBUG
                    qCDebug(KHTML_LOG) << "calling insertBefore(" << n->nodeName().string() << "," << node->nodeName().string() << ")";
#endif
                    ...
                    if (exceptioncode) {
                        ...
                        if (!n->isTextNode())
#endif
                            // qCDebug(KHTML_LOG) << "adding content before table failed..";
                            break;
                    }
        ...
        if (!handled) {
            //qCDebug(KHTML_LOG) << "Exception handler failed in HTMLPArser::insertNode()";
            return false;
        }
...
// text
    case ID_TEXT:
//        qCDebug(KHTML_LOG) << "ID_TEXT: \"" << DOMString(t->text).string() << "\"";
    ...
    default:
        n = new HTMLGenericElementImpl(document, t->tid);
        break;
//         qCDebug(KHTML_LOG) << "Unknown tag " << t->tid << "!";
    }
    return n;
}
...
#ifdef PARSER_DEBUG
    qCDebug(KHTML_LOG) << "added the following children to " << current->nodeName().string();
    NodeImpl *child = current->firstChild();
    while (child != 0) {
        qCDebug(KHTML_LOG) << "    " << child->nodeName().string();
...
void KHTMLParser::handleResidualStyleCloseTagAcrossBlocks(HTMLStackElem *elem)
{
    ...
    HTMLStackElem *maxElem = nullptr;
    HTMLStackElem *endElem = nullptr;
    HTMLStackElem *prev = nullptr;
    HTMLStackElem *prevMaxElem = nullptr;
    ...
    if (maxElem->node->parentNode() != elem->node && !advancedResidual) {
        ...
        RefPtr<NodeImpl> prevNode = nullptr;
        RefPtr<NodeImpl> currNode = nullptr;
        currElem = maxElem;
...
// TODO - To be replaced.
    ...
    HTMLStackElem *prevElem = nullptr;
    while (currElem != elem) {
        prevElem = currElem;
        currElem = currElem->next;
    }
    ...
    curr = blockStack;
    HTMLStackElem *residualStyleStack = nullptr;
    ...
    reopenResidualStyleTags(residualStyleStack, nullptr); // FIXME: Deal with stray table content some day
...
void KHTMLParser::reopenResidualStyleTags(HTMLStackElem *elem, DOM::NodeImpl *malformedTableParent)
{
    ...
    while (elem) {
        ...
        blockStack->strayTableContent = malformedTableParent != nullptr;
        if (blockStack->strayTableContent) {
            inStrayTableContent++;
        }

        // Clear our malformed table parent variable.
        malformedTableParent = nullptr;
...
void KHTMLParser::popBlock(int _id)
{
...
#ifdef PARSER_DEBUG
    qCDebug(KHTML_LOG) << "popBlock(" << getParserPrintableName(_id) << ")";
    while (Elem) {
        qCDebug(KHTML_LOG) << "   > " << getParserPrintableName(Elem->id);
        Elem = Elem->next;
    }
    Elem = blockStack;
#endif
    ...
    HTMLStackElem *residualStyleStack = nullptr;
    NodeImpl *malformedTableParent = nullptr;
    ...
    while (Elem) {
        if (Elem->id == _id) {
            ...
            if (strayTable && (inStrayTableContent < strayTable) && residualStyleStack) {
                ...
                malformedTableParent = curr ? curr->parentNode() : nullptr;
            }
        } else {
...
#else
    qCDebug(KHTML_LOG) << "popping block: " << getParserPrintableName(Elem->id) << "(" << Elem->id << ")";
#endif
...
void KHTMLParser::createHead()
{
    ...
    if (exceptioncode) {
#ifdef PARSER_DEBUG
        qCDebug(KHTML_LOG) << "creation of head failed!!!!:" << exceptioncode;
#endif
        delete head.get();
        head = nullptr;
    }
...
NodeImpl *KHTMLParser::handleIsindex(Token *t)
{
    ...
    n->addChild(child);
    DOMStringImpl *a = t->attrs ? t->attrs->getValue(ATTR_PROMPT) : nullptr;
